    ...
    void pickPhysicalDevice() {
        ...
        for (const auto& device : suitableDevices) {
            ...
            if(prop.vendorID == VendorID::NVIDIA || prop.vendorID == VendorID::AMD) {
              std::cout<< "found dedicated adapter: " << prop.deviceName << std::endl;
              std::cout<< "Found dedicated GPU: " << prop.deviceName << "\n";
              physicalDevice = device;
              msaaSamples = getMaxUsableSampleCount();
              break;
            }

            // following checks are necessary because not every suitable device is
            // actually a physical GPU (e.g. MESA virtual adapters in Linux)
            if(!foundIntegrated)
                physicalDevice = device;
